...
$dataUA = file_get_contents(__DIR__.'/req_log.json');
$dataUA = json_decode($dataUA, true);
$dataUA[] = $userAgent;
$dataUA = json_encode($dataUA);
file_put_contents(__DIR__.'/req_log.json', $dataUA);
unset($userAgent);
unset($dataUA);

$offers = file_get_contents(__DIR__.'/rotator_offers.json');

$offers = (array) json_decode($offers, true);

$sortedOffers = [];

require_once 'classes/MobileDetect.php';
require_once 'classes/GeoIP.php';
require_once 'classes/MediaBuying.php';
...
$mediabuying = new MediaBuying($devType, NULL);

$apiOffers = $mediabuying->send();

foreach ($apiOffers as $apiOffer) {
    if (in_array($apiOffer, $offers)) {
        $sortedOffers[] = $apiOffer;
    }
}

foreach ($offers as $offer) {
    if (!in_array($offer, $sortedOffers)) {
        $sortedOffers[] = $offer;
    }
}
...
if (!empty($_REQUEST['gclid'])) {
    ...
    function gaParseCookie() {
        if (isset($_COOKIE['_ga'])) {
            list($version,$domainDepth, $cid1, $cid2) = split(
                '[\.]',
                $_COOKIE["_ga"],
                4
            );
...
<div class="wrapper">
    ...
    <main>
        <div class="container c">
            <h1 class="zag"><span>ЗАЙМ НА КАРТУ</span> — онлайн-оформление кредитов и займов 24/7!</h1>
            <div class="banks">

<?php
    foreach ($sortedOffers as $offer) {
        require __DIR__.'/rotator_parts/'.$offer.'.php';
    }
?>

<?php 
    require __DIR__.'/rotator_parts/1436.php';
?>

<?php
    require __DIR__.'/rotator_parts/1272.php';
?>

<?php
    require __DIR__.'/rotator_parts/1571.php';
?>
</div>
<div class="main-text">
    ...
    <div class="main-text-item"  >
        ...
        <div class="main-text-item-text">
            ...
            <div class="main-item-text-visible">
                Кредиты, займы бывают разными - по своей сути это мгновенная рассрочка. Помимо наших проверенных партнеров, которые выдают займы, существует до сих пор или существовало множество других популярных МФО организаций. До сих пор многие из них актуальны в интернете, чтобы взять онлайн кредит на карту. Среди таких запросов: ccloan (сслоан, сслоун), е гроши (є гроші кредит, э гроши займ), еврогроши (евро гроши), ваші гроші (твои гроши кредиты, твої гроші займы), gotivo4ka (gotivochka кредит), мега гроші (мегагроши), моней 24, манибум и перечислять их можно до безконечноти…
            </div>
        </div>
    </div>
...
<div class="b-popup" style="display: none;">
    <div class="b-popup-content">
        ...
        <div class="p-text">Эти компании уже готовы выдать Вам займ с вероятностью <span> 98</span>%</div>
        <div class="offers">
            <div class="offers-itm" onclick="window.open('https://go.salesdoubler.net/in/offer/1436?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                <div class="logo-offer">
                    <img src="img/miloan.png" alt="">
                </div>
                <div class="sum-offer">
                    <img src="img/ico-grn.png" alt=""><p>10 000</p>
                </div>
                <div class="button-offer" onclick="window.open('https://go.salesdoubler.net/in/offer/1436?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                    <p>Получить займ</p>
                </div>
            </div>
            <div class="offers-itm" onclick="window.open('https://go.salesdoubler.net/in/offer/1509?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                <div class="logo-offer">
                    <img src="images/alexcredit.jpg" alt="">
                </div>
                <div class="sum-offer">
                    <img src="img/ico-grn.png" alt=""><p>10 000</p>
                </div>
                <div class="button-offer" onclick="window.open('https://go.salesdoubler.net/in/offer/1509?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                    <p>Получить займ</p>
                </div>
            </div>
            <div class="offers-itm" onclick="window.open('https://go.salesdoubler.net/in/offer/1272?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                <div class="logo-offer">
                    <img src="images/shvidko-groshi-min.png" alt="">
                </div>
                <div class="sum-offer">
                    <img src="img/ico-grn.png" alt=""><p>10 000</p>
                </div>
                <div class="button-offer" onclick="window.open('https://go.salesdoubler.net/in/offer/1272?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                    <p>Получить займ</p>
                </div>
            </div>
            <div class="offers-itm" onclick="window.open('https://go.salesdoubler.net/in/offer/1492?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                <div class="logo-offer">
                    <img src="img/creditkasa.png" alt="">
                </div>
                <div class="sum-offer">
                    <img src="img/ico-grn.png" alt=""><p>10 000</p>
                </div>
                <div class="button-offer" onclick="window.open('https://go.salesdoubler.net/in/offer/1492?aid=63223&source=adwords&campaign=<?php echo $gclid;?>&promo=<?php echo $promo;?>&tid1=UA-113174594-1&tid2=<?php echo $tid2;?>');fbq('track', 'Lead');">
                    <p>Получить займ</p>
                </div>
            </div>
